...
export default function ExportView({ database }: ExportViewProps) {
    ...
    const handleExport = async () => {
        ...
        setError("");
        try {
            const result = await api.exportDatabase(database, format, includeStructure, includeData);
            const blob = new Blob([result], { type: format === "json" ? "application/json" : format === "csv" ? "text/csv" : "text/sql" });
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url;
            a.download = `${database}_export.${format}`;
            a.click();
