...
import { Database, Table, ChevronRight, ChevronDown, LogOut, Plus, Check, X, Sun, Moon, Eye, Terminal, Variable, Users, Activity, Share2, Clock, Settings } from "lucide-react";
...
export default function Sidebar({ onSelectTable, onSelectObject, onSelectDb, onSelectUsers, onSelectPerformance, onSelectSlowLog, onSelectDiagram, selectedDb, selectedTable, selectedObject }: SidebarProps) {
    ...
    const fetchDbObjects = async (db: string) => {
        ...
        try {
            // Fetch tables first
            const tableData = await api.listTables(db);
            ...
            try {
                const objectData = await api.getObjects(db);
        ...
        <aside className={styles.sidebar}>
            <div className={styles.sidebarHeader}>
                ...
                <div className={styles.headerActions}>
                    <button onClick={() => navigate("/settings")} className={styles.themeBtn} title="LLM Settings">
                        <Settings size={18} />
                    </button>
